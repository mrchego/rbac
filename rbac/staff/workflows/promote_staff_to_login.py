# rbac/staff/workflows/promote_staff_to_login.py
...
def promote_staff_to_login(*, user_id, company, role, invited_by, request):
    ...
    if user.can_login:
        raise ApplicationError("This staff member can already log in.", code=ErrorCode.VALIDATION_ERROR)

    if Invitation.objects.filter(email=user.email, company=company, used=False).exists():
        raise ApplicationError("An invitation is already pending for this staff member.", code=ErrorCode.VALIDATION_ERROR)

    UserRole.objects.create(user=user, role=role)

    invitation = Invitation.objects.create(
        email=user.email, company=company, invited_by=invited_by, role=role,
    )
